...
- **Every turn**, `build_frame_block(load_stats)` emits the non-neutral modifiers into
  ...
  right stat set + rule strings). At the User default, EDGE/NERVE/IRON/SIGNAL all fire.
...
- **Overrides:** make `layla stat NERVE 9` / the `/operator/profile/stat` endpoint
  (`routers/settings.py:667-688`) accept the new ids, and seed the User default so an out-of-box install is
...
2. **V1/V4 (the real fix):** build EDGE/NERVE/IRON/SIGNAL into `frame_modifier.py` + `operator_quiz.py`,
   seed the User default, add the SIGNAL global length clamp. This makes the edge *durable* across aspects,
