...
# Model + config repair is done in PYTHON (robust JSON + path handling; the PowerShell versions kept
# tripping over ~ expansion and huge-config serialization). It finds the model wherever it landed
# (including a mis-placed literal "~" folder), moves it into a writable folder, and writes models_dir +
# model_filename correctly. Pass -DownloadModel <catalog-name> to fetch one if none is found.
$configChanged = $false
$prevEAP = $ErrorActionPreference
$ErrorActionPreference = 'Continue'
try {
  $rm = Join-Path $env:TEMP "layla_repair_model.py"
  Invoke-WebRequest "https://raw.githubusercontent.com/PapaKoftes/Layla/master/installer/repair_model.py" -OutFile $rm -UseBasicParsing
  & $Python $rm $agent $env:LAYLA_DATA_DIR $DownloadModel
  if ($LASTEXITCODE -eq 0) {
    $configChanged = $true
    Write-Host "Model   : configured." -ForegroundColor Green
  } else {
    Write-Host "Model   : none found. Re-run with  -DownloadModel 'dolphin-3.0-llama3.1-8b-Q4_K_M'  (or a smaller one like 'tinydolphin-1.1b-Q4_K_M')." -ForegroundColor Yellow
  }
} catch { Write-Host "  (model repair step skipped: $_)" -ForegroundColor DarkYellow }
finally { $ErrorActionPreference = $prevEAP }
