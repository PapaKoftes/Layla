...
Curated domain knowledge now lives in `knowledge/packs/<id>/` — each pack has a `pack.json`
manifest and docs with `priority`/`domain`/`aspect`/`summary` front matter. `registry.json`
and `presets.json` are generated by `agent/scripts/build_knowledge_registry.py` and gated by
`test_knowledge_packs.py`. Design + rollout: `docs/design/KNOWLEDGE_PRESETS.md`.
Loose files in this folder are still supported.
