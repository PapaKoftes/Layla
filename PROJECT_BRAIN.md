...
- **Core loop:** **`agent/agent_loop.py`** + **`execution_state.py`**; **`services/coordinator.py`**, **`services/prompt_builder.py`**, **`services/memory_consolidation.py`**; config **`agent/runtime_safety.py`**; tools **`agent/layla/tools/registry.py`** + **`agent/layla/tools/impl/*.py`**. **SQLite:** **`layla/memory/migrations.py`** (schema) + domain modules (`learnings.py`, `plans_db.py`, **`tasks_db.py`**, …) re-exported from **`layla/memory/db.py`**. **Packaged / per-user paths:** **`LAYLA_DATA_DIR`** → `runtime_config.json`, `layla.db`, `models/` (see **`runtime_safety.py`**, **`layla/memory/db_connection.py`**). **Background tasks:** **`services/agent_task_runner.py`**; HTTP surface split as **`routers/learn.py`** + **`routers/agent_tasks.py`** (included from **`routers/agent.py`**).
