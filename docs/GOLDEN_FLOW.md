...
4. **Response**: Router builds JSON with `response`, `state` (steps, status), `reasoning_mode`, `conversation_id`, etc. On success it appends to session history and calls `create_conversation` / `append_conversation_message` in SQLite when possible.
