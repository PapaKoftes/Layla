...
2. **Partial architectural extraction** — `agent/core/loop.py` (planned, not present): `run_loop` only builds a snapshot via `core.observer` and returns; phases 2–6 remain in `agent_loop` (see module docstring).
...
**Removed stub:** `core/loop.py` was deleted (dead `run_loop()` hook). The live path calls `core.observer.build_snapshot` directly inside `agent_loop.py`.
...
| [AGENTS.md](../AGENTS.md) | **195 tools** — matches `EXPECTED_TOOL_COUNT` |
...
**Stale tool counts in `knowledge/`:** Scan for literal `109` in `knowledge/**/*.md` (2026-04-03): **no matches**. Re-scan for stale counts when bumping `EXPECTED_TOOL_COUNT`; sample product docs should track **195** (same as `test_registered_tools_count.py`).
...
- **Re-run knowledge stale scan:** `rg '\\b109\\b' knowledge --glob '*.md'` (legacy tool count); also `rg '\\b179\\b'` after bumps to `EXPECTED_TOOL_COUNT` (currently **195**).
