...
| 12 | Decision system: feasibility, knowledge depth, alignment, creativity, risk; execution via Morrigan | `orchestrator.build_deliberation_prompt` with structured roles; CONCLUSION — MORRIGAN; bias extension augments JSON decision path; decision prompt few-shot; resilient JSON extraction/repair (`decision_schema.py`); optional `decision_model` routing for decision JSON | Deliberation prompt format; agent loop tests |
