...
**Layout (2026 consolidation):** SQLite schema in **`agent/layla/memory/migrations.py`**; table APIs in domain modules re-exported from **`db.py`** (includes **`tasks`** via **`tasks_db.py`** for coordinator persistence). Agent HTTP: **`routers/agent.py`** composes **`learn.py`** + **`agent_tasks.py`**; outer entry **`services/coordinator.run`** → **`dispatch_autonomous_run`**. **`POST /agent/persistent_tasks/{id}/resume`** re-enters with **`execution_state_json`**. In-loop plans: **`planner.execute_plan_with_optional_graph`** (task graph when enabled). Background workers in **`services/agent_task_runner.py`**; scheduled consolidation/reflection in **`main.py`** (`memory_consolidation`, `background_intelligence`). Tiered prompt caps: **`services/prompt_tier_budget.py`**. System-head static layers: **`services/prompt_builder.py`**.
...
| 5 | Workflow translation: Geometry→Fabrication→Machine intent; DXF→machinable, parametric→geometry, Python→automation | **PARTIAL, extended MVP:** deterministic **`layla.geometry.machining_ir`** + **`geometry_extract_machining_ir`**; `generate_gcode` now supports DXF **LINE/LWPOLYLINE/ARC/CIRCLE** + stronger **`validate_gcode_text`**; `cam/simulator.simulate_gcode`; bundle tool **`cam_build_machine_intent`** (IR + feeds/speeds + validation + simulation); `validate_fabrication_bundle`; rule-based **`layla/cam/`** tools (`cam_feed_speed_hint`, `cam_estimate_time`, `cam_list_tool_types`). Still **not** full CAM/post/stock/collision — **[FABRICATION_IR_AND_TOOLCHAIN.md](FABRICATION_IR_AND_TOOLCHAIN.md)** | `test_machining_ir.py`; **`test_wave4_toolchain_cam.py`**; geometry + fabrication domains |
| 6 | Execution loop: Learn→Plan→Assist→Evaluate→Improve; applied learning | **Implemented (heuristic loop):** `agent_loop` computes structured eval and stores it on `state["outcome_evaluation"]`; last eval persisted via SQLite `outcome_evaluations` + `shared_state` fallback; planner bias reads last eval; outcome memory + reflection + tool-pattern learnings + distill + bounded low-confidence prune. | `test_outcome_evaluation.py`; `test_planner_outcome_bias.py`; `test_companion_intelligence.py`; scheduler |
...
| 8 | Failure awareness: workflow breakdowns, planning gaps, execution issues; assist recovery | **`recovery_strategy`:** `replan` \| `retry_constrained` \| `escalate_user` on `recovery_hint`; enriched `failure_type` (tool/planning/reasoning/context/parse/execution) in `services/failure_recovery.py`; deterministic tool verification feeds `environment_aligned`; prompt **Next step** line in `format_recovery_hint_for_prompt`; replan nudge in `_llm_decision` when strategy `replan` and `should_plan` false. | agent loop + governance tests; `test_agent_loop.py` |
...
| 10 | Initiative model: suggest improvements, propose projects, explore safely; gated | **PARTIAL:** Wakeup initiative + **`services/initiative_engine.py`** when **`initiative_engine_enabled`** (merges into **`initiative_inline`** + optional wakeup). **`inline_initiative_enabled`**. **v3:** `/improvements/*`. | `test_initiative_autonomy.py`; `test_wakeup_initiative_suggestion`; `test_self_improvement.py` |
...
| 12 | Decision system: feasibility, knowledge depth, alignment, creativity, risk; execution via Morrigan | `services/intent_router.py` provides a single RouteDecision (task_type + hints) consumed by `agent_loop._llm_decision`; tool preflight prevents unrunnable tool picks; decision prompt few-shot (with meta/self-question bias toward `action="reason"` and explicit `reason` visibility); resilient JSON extraction/repair (`decision_schema.py`); optional `decision_model` routing for decision JSON | Deliberation prompt format; agent loop tests |
| 13 | Identity continuity: evolve, consistency, quirks; Echo tracks long-term growth | Echo prompt; style_profile; learnings; aspect_memories; `human_aligned` bias nudge in decisions; optional **`voice_adjustment_inject_enabled`** + **`user_identity.voice_adjustment`** in **`_build_system_head`** | Wakeup; Echo in deliberation |
| 14 | Autonomy: suggest, guide, organize; eventually initiate safely | **PARTIAL:** Wakeup + inline + initiative engine (suggestions only). **`autonomy_optimizer_enabled`** adds **text-only** recovery hints inside **`run_governed_plan_step`** retries (never widens allow_write/allow_run). Approval flow unchanged. | `test_initiative_autonomy.py`; approval tests |
...
| 17 | Toolchain awareness: format transitions, workflow dependencies, automation paths | **PARTIAL:** **`services/toolchain_awareness`** + **`services/toolchain_graph`** (`suggest_cheaper_path`, **`planner_toolchain_cost_line`** in **`build_planning_bias_prompt`**) + IR doc **[FABRICATION_IR_AND_TOOLCHAIN.md](FABRICATION_IR_AND_TOOLCHAIN.md)** | `test_wave4_toolchain_cam.py`; in-head + planner bias |
...
| 19 | Long-term growth: capability, alignment, partnership | Capabilities + domains; learnings; study plans; usefulness-weighted growth. **v3:** maturity XP/rank/phase (`services/maturity_engine.py`) with wakeup continuity (`/wakeup`). | Capability events; `test_wakeup_continuity.py` |
...
| Windows packaged install + launcher | `launcher/layla_launcher.py`, `launcher/layla.spec`, `installer/layla.iss`, `installer/build_installer.ps1`; env `LAYLA_DATA_DIR` / `LAYLA_INSTALL_ROOT`; `runtime_safety` config + `default_models_dir()`; `layla/memory/db_connection.py` → `layla.db`; Web setup overlay (`layla-app.js`); `services/release_updater.py`; `POST /update/apply` branch | Release ZIP must contain extractable `agent/` tree; `github_repo` in config |
