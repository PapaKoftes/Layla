...
| MCP | Full MCP *client* in agent | IMPLEMENTED (`mcp_session_call_tool`, `mcp_session_list_tools`, `mcp_session_list_resources`, `mcp_session_read_resource`; registry `mcp_tools_call`, `mcp_list_mcp_tools`, `mcp_list_mcp_resources`, `mcp_read_mcp_resource`, `mcp_operator_auth_hint`; optional `mcp_inject_tool_summary_in_decisions` TTL block in `_llm_decision`; opt-in via `mcp_client_enabled` / `mcp_stdio_servers`) |
