...
def start_service(cfg: dict | None = None) -> bool:
    ...
    with _lock:
        ...
        try:
            ...
            # Register in a daemon thread to avoid EventLoopBlocked on the
            # synchronous startup path — zeroconf's internal async loop must
            # not share the caller's thread.
            reg_err: list[Exception] = []
