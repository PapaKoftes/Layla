...
def _sm2(ease_factor: float, interval: int, reps: int, quality: int) -> tuple[float, int, int]:
    """SM-2 step for the flashcard deck — a thin adapter over the ONE canonical implementation.

    quality: 0-5 (0-2 = fail, 3-5 = pass). Returns (new_ease_factor, new_interval, new_reps).

    The algorithm used to be hand-rolled here (and separately in the learnings path, and in a since-
    deleted module), and the copies had drifted apart on real behaviour. It now lives once in
    services.memory.spaced_repetition.sm2, so a correctness fix lands in a single place. Imported
    lazily per call to match this module's import style and to keep the canonical the single source
    of truth even under test monkeypatching.
    """
    from services.memory.spaced_repetition import sm2
    return sm2(ease=ease_factor, interval_days=interval, reps=reps, quality=quality)
