...
def is_ip_allowed(ip: str, cfg: dict) -> bool:
    ...
    # NOTE: no "localhost is always allowed" short-circuit. is_ip_allowed is only
    # reached for REMOTE requests — the middleware exempts genuine direct-local
    # before auth runs. A request whose IP merely parses as 127.0.0.1 here came
    # from a (spoofable) forwarding header, so it must satisfy the allowlist like
    # any other remote IP.
    try:
