...
def _get_chroma_collection():
    ...
    # Check for dimension mismatch between stored embeddings and current embedder
    try:
        if coll.count() > 0:
            ...
            if stored_embs and stored_embs[0]:
                ...
                _get_embedder()  # ensure _embedder_dim is set
        ...
        _vs_log.debug("ChromaDB dimension check failed: %s", _dim_e)
...
def rebuild_collection() -> dict:
    ...
    try:
        ...
        try:
            ...
            _vs_log.info("ChromaDB: deleted old 'learnings' collection for rebuild")
        except Exception as _del_e:
            _vs_log.debug("ChromaDB delete collection failed (may not exist): %s", _del_e)
        _chroma_collection = None
        coll = _get_chroma_collection()
        # Re-index from SQLite learnings
        count = 0
        try:
            ...
            _vs_log.warning("rebuild re-index failed: %s", _idx_e)
        _vs_log.info("ChromaDB rebuild complete: %d documents re-indexed", count)
        return {"ok": True, "indexed": count}
    except Exception as e:
        _vs_log.warning("rebuild_collection failed: %s", e)
