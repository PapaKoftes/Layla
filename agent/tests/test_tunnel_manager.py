...
class TestTunnelLifecycle:
    def test_status_start_stop(self):
        ...
        from services.infrastructure.tunnel_manager import start_quick_tunnel, stop_tunnel, tunnel_status
        with patch("shutil.which", return_value=None):
            status = tunnel_status()
            assert isinstance(status, dict)
...
class TestHealthCheck:
    def test_not_running_returns_unhealthy(self):
        ...
        import services.infrastructure.tunnel_manager as tm
        tm._proc = None
        tm._last_url = ""
...
class TestGetHealthState:
    def test_returns_dict(self):
        from services.infrastructure.tunnel_manager import get_health_state
