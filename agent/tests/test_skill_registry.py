...
class TestRegister:
    def test_register_basic(self):
        from services.skills.skill_registry import get_pack, register
...
class TestUnregister:
    def test_unregister_existing(self):
        from services.skills.skill_registry import get_pack, register, unregister
        register("doomed", "1.0", "/path")
        assert unregister("doomed") is True
        assert get_pack("doomed") is None

    def test_unregister_nonexistent(self):
        from services.skills.skill_registry import unregister
        assert unregister("nope") is False


class TestListPacks:
    def test_empty(self):
        from services.skills.skill_registry import list_packs
        assert list_packs() == []

    def test_lists_all(self):
        from services.skills.skill_registry import list_packs, register
        register("a", "1.0", "/a")
        register("b", "2.0", "/b")
        packs = list_packs()
...
class TestHealthStatus:
    ...
    def test_update_health(self):
        from services.skills.skill_registry import get_pack, register, update_health
...
class TestLastRun:
    def test_update_last_run(self):
        from services.skills.skill_registry import get_pack, register, update_last_run
