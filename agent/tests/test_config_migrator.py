...
class TestMigrateConfig:
    ...
    def test_full_config_no_changes(self):
        from services.infrastructure.config_migrator import _NEW_DEFAULTS, migrate_config
...
class TestMigrationStatus:
    def test_needs_migration(self):
        from services.infrastructure.config_migrator import get_migration_status
        status = get_migration_status({})
        assert status["needs_migration"] is True
        assert status["pending_changes"] > 0

    def test_no_migration_needed(self):
        from services.infrastructure.config_migrator import _NEW_DEFAULTS, get_migration_status
        cfg = dict(_NEW_DEFAULTS)
...
class TestGetVersion:
    def test_returns_version(self):
        from services.infrastructure.config_migrator import get_current_version
