...
def fresh_db(tmp_path):
    ...
    from services.workspace.repo_indexer import migrate
    migrate(db_path=db)
    return db
...
def test_extract_symbols_ast_finds_class_and_function():
    from services.workspace.repo_indexer import _extract_symbols_ast
    source = "class Foo:\n    def bar(self): pass\n\ndef baz(): pass\n"
...
def test_index_file_idempotent(tmp_path, fresh_db):
    ...
    from services.workspace.repo_indexer import get_symbols, index_file
    py = tmp_path / "idm.py"
...
def test_search_symbols_finds_partial_match(tmp_path, fresh_db):
    from services.workspace.repo_indexer import index_file, search_symbols
...
def test_get_callers_of(tmp_path, fresh_db):
    from services.workspace.repo_indexer import get_callers_of, index_file
...
def test_get_symbol_context_returns_string(tmp_path, fresh_db):
    from services.workspace.repo_indexer import get_symbol_context, index_file
    py = tmp_path / "ctx.py"
...
def test_skip_dirs_ignored(tmp_path, fresh_db):
    ...
    index_workspace_repo(tmp_path, db_path=fresh_db)
    from services.workspace.repo_indexer import get_symbols
