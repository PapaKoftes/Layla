...
def test_captures_explicit_facts():
    ...
    assert dict(ex("call me User")).get("name") == "User"
...
def test_capture_writes_and_returns_receipt(tmp_path, monkeypatch):
    ...
    receipt = capture_identity_from_turn("call me User and i use neovim")
    assert saved.get("name") == "User" and saved.get("editor") == "Neovim"
    assert "what i know about you" in receipt.lower()
    # nothing new → empty receipt (idempotent, no redundant writes)
    monkeypatch.setattr(db, "get_all_user_identity", lambda: {"name": "User"})
    assert capture_identity_from_turn("call me User") == ""
