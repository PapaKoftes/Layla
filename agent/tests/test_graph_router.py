    ...
    with p1, p2, p3:
        _seed(
            ...
            nodes=[
                ...
                (2, "Mina ships on Fridays", {"source": "timeline"}, "2026-03-01T00:00:00"),
            ],
        ...
        assert labels == ["Mina ships on Fridays", "Project Castilla", "Ahmed prefers email"]
        ...
        assert by_label["Mina ships on Fridays"]["degree"] == 0
...
def test_router_reads_what_the_real_writers_wrote(client, tmp_path):
    ...
    with p1, p2, p3, \
        ...
        a = mg.add_node("Mina builds Layla", {"source": "conversation"})
        b = mg.add_node("Layla runs locally", {"source": "conversation"})
        mg.add_edge(a, b, "relates_to")

        listing = client.get("/graph/entities").json()
        assert listing["total"] == 2
        assert {e["label"] for e in listing["entities"]} == {"Mina builds Layla", "Layla runs locally"}
