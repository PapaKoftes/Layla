...
class TestCrawlBasic:
    @patch("services.retrieval.web_crawler._BACKENDS")
    def test_basic_fallback(self, mock_backends):
        from services.retrieval.web_crawler import crawl_url
        def mock_fn(url, cfg):
...
class TestCrawlFirecrawl:
    def test_firecrawl_import_guard(self):
        ...
        from services.retrieval.web_crawler import _crawl_firecrawl
...
class TestCrawlCrawl4ai:
    def test_crawl4ai_import_guard(self):
        ...
        from services.retrieval.web_crawler import _crawl_crawl4ai
        result = _crawl_crawl4ai("http://example.com", {})
        assert result["ok"] is False


class TestCrawlUrls:
    @patch("services.retrieval.web_crawler.crawl_url")
    def test_multiple_urls(self, mock_crawl):
        from services.retrieval.web_crawler import crawl_urls
...
class TestAutoDetect:
    def test_auto_falls_to_basic(self):
        ...
        import services.retrieval.web_crawler as wc
        from services.retrieval.web_crawler import _resolve_backend
...
class TestContentTruncation:
    def test_long_content_truncated(self):
        ...
        from services.retrieval.web_crawler import _truncate
        long_text = "x" * 100000
