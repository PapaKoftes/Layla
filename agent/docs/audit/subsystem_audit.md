...
| Reranker | PRESENT | Implemented at `agent/services/retrieval/reranker.py` (cross-encoder retrieval reranking) with tests. (This row previously read MISSING — stale.) | Plan Phase 4.2 ("Retrieval Ranking by Confidence") shipped. |
...
| Debate / Council / Tribunal | PRESENT | Implemented at `agent/services/planning/debate_engine.py` (multi-aspect deliberation) with tests; gated by `deliberation_enabled`. (This row previously read MISSING — stale.) | North-star "first-class"; ships as an opt-in deliberation mode. |
...
15. **Stale-doc sweep.** `agent/docs/` is 80+ markdown files, many overlapping (`AUDIT_2025-03-14.md`, `INTERNAL_NOTE.md`, `MODULE_SWEEP_*`, `*_SECOND_SWEEP.md`). Decide which are canonical; archive the rest under `docs/archive/`. Effort: 2-3 h. Impact: reduces collaborator confusion.
