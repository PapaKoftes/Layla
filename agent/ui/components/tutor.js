...
function _onDocKeydown(e) {
  if (!_open) return;
  if (e.key === "Escape") { e.preventDefault(); e.stopPropagation(); closeTutor(); }
}
...
function _build() {
  ...
  _root.innerHTML =
    '<div class="cmdp-panel sysdiag-panel german-panel" role="document">' +
      '<div class="cmdp-search-row"><span class="cmdp-search-icon tutor-flag" aria-hidden="true">🌐</span>' +
        '<span class="sysdiag-title" data-i18n="nav.language_tutor">Language tutor</span>' +
  ...
  if (_escChip) {
    ...
    _escChip.addEventListener("click", () => closeTutor());
    _escChip.addEventListener("keydown", (e) => { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); closeTutor(); } });
  }
