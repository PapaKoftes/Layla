/**
 * components/chat-render.js — Chat rendering, typing indicators, streaming UI,
 * message display, and stream statistics.
 *
 * Converted from js/layla-chat-render.js (IIFE -> ES module).
 * Depends on: services/utils.js (escapeHtml, showToast, sanitizeHtml, cleanLaylaText)
 *             components/aspect.js (facetMetaFromNameOrId, formatLaylaLabelHtml)
 *
 * Heavy cross-module deps resolved at call-time via window.* guards:
 *   fetchWithTimeout, toggleSendButton (self-ref via window for retryLastMessage),
 *   executePlan, refreshApprovals, LaylaUI, hljs, marked
 */

import { escapeHtml, showToast, sanitizeHtml, cleanLaylaText } from '../services/utils.js';
import { facetMetaFromNameOrId, formatLaylaLabelHtml } from '../components/aspect.js';
...
export function laylaNotifyStreamPhase(row, uxKey) {
  try {
...
export function laylaApplyUiTimeoutsFromHealth(d) {
  if (!d) return;
  try {
...
export function laylaAgentStreamTimeoutMs() {
  var t = window.__laylaUiTimeouts;
  return t && t.streamMs > 0 ? t.streamMs : 900000;
}

export function laylaAgentJsonTimeoutMs() {
  var t = window.__laylaUiTimeouts;
  return t && t.jsonMs > 0 ? t.jsonMs : 720000;
}

export function laylaStalledSilenceMs() {
  var t = window.__laylaUiTimeouts || {};
...
// ── Header progress bar ───────────────────────────────────────────────────────
export function laylaHeaderProgressStart() {
...
export function laylaHeaderProgressStop() {
  var row = document.getElementById('header-progress-row');
  var fill = document.getElementById('header-progress-fill');
  if (row) {
...
// ── Operator trace log ────────────────────────────────────────────────────────
export function operatorTraceClear() {
  var b = document.getElementById('operator-trace-log');
  if (b) b.innerHTML = '';
}

export function operatorTraceLine(kind, text) {
  var b = document.getElementById('operator-trace-log');
  if (!b) return;
...
export function laylaStreamStatsStart(modelName) {
...
export function laylaStreamStatsStep(label) {
  if (!_streamStatsActive) return;
  _streamStepCount++;
  _updateStreamStepEl();
...
export function toggleComposePanel(force) {
...
export function laylaRunPlanFromElement(el) {
  if (!el) return;
  ...
  try {
    ...
    showToast('Invalid JSON — fix the plan text');
    return;
  }
  if (!Array.isArray(plan)) {
    showToast('Plan must be a JSON array of steps');
    return;
  }
  if (typeof window.executePlan === 'function') window.executePlan(plan, goal);
}

export function laylaFormatPlanJson(btn) {
...
export function _guessPathFromCodeBlock(text) {
...
export async function _laylaApprovePendingForCodeBlock(codeText) {
  var fetchFn = (typeof window.fetchWithTimeout === 'function') ? window.fetchWithTimeout : fetch;
  var res = await fetchFn('/pending', {}, 8000);
  ...
  if (!r.ok || !body.ok) {
    showToast((body && body.error) ? String(body.error) : ('Approve failed: ' + r.status));
    return;
  }
  showToast('Applied');
  try { if (typeof window.refreshApprovals === 'function') window.refreshApprovals(); } catch (_) {}
...
export function _addApplyBtnToCodeBlock(wrap, codeEl) {
...
export function renderPromptTilesAndEmptyState() {
...
export function _renderReasoningTreeSummary(container, summary) {
...
export function addMsg(role, text, aspectName, deliberated, steps, uxStates, memoryInfluenced, reasoningTreeSummary) {
  ...
  if (role === 'layla') {
    ...
    correctBtn.className = 'msg-correct-btn';
    correctBtn.type = 'button';
    correctBtn.textContent = 'correct';
    correctBtn.title = 'Correct a fact in this message';
    correctBtn.setAttribute('aria-label', 'Correct a fact');
    correctBtn.onclick = function (ev) {
      ev.stopPropagation();
...
export function _renderDeliberationTranscript(msgDiv, meta) {
  ...
  var labelDelib = MODE_LABELS[meta.mode] || ('✦ ' + meta.mode);
  var summaryEl = document.createElement('summary');
  summaryEl.className = 'deliberation-label';
  summaryEl.innerHTML = labelDelib + ' — <span style="opacity:0.7">' + aspects.length + ' voices</span>';
...
export function laylaUpdateTypingUx(uxKey) {
...
export function laylaRemoveTypingIndicator() {
...
export function laylaShowTypingIndicator(aspectId, initialUxKey) {
  hideEmpty();
...
export function laylaEnsureReasoningChain(msgLaylaDiv) {
...
export function laylaAppendReasoningStep(msgLaylaDiv, text, stepNum) {
...
export function laylaSyncChatChromeFromFSM(state) {
  try {
...
// laylaOnChatState callback — installed via initChatRender
export function laylaOnChatState(st) {
...
// ── Fact correction UI (Gap 6) ────────────────────────────────────────────────

export function openFactCorrectionForm(bubble, btn) {
  // Check if form already exists
  var existing = bubble.parentElement.querySelector('.fact-correction-form');
  if (existing) { existing.remove(); return; }

  var form = document.createElement('div');
  form.className = 'fact-correction-form';
  form.style.cssText = 'margin-top:8px;padding:10px;background:var(--code-bg);border:1px solid var(--border);border-radius:8px;font-size:0.72rem;';
  form.innerHTML =
    ...
    '</div>' +
    '<div class="fc-status" style="margin-top:6px;font-size:0.65rem;color:var(--text-dim)"></div>';

  // Pre-fill with selected text if any
  var sel = window.getSelection();
  if (sel && sel.toString().trim() && bubble.contains(sel.anchorNode)) {
    form.querySelector('.fc-query').value = sel.toString().trim();
  }

  form.querySelector('.fc-cancel').onclick = function () { form.remove(); };
  form.querySelector('.fc-submit').onclick = function () {
    submitFactCorrection(form);
  };

  bubble.parentElement.appendChild(form);
}

export function submitFactCorrection(form) {
  var query = (form.querySelector('.fc-query').value || '').trim();
  var correction = (form.querySelector('.fc-correction').value || '').trim();
  var status = form.querySelector('.fc-status');
  if (!query || !correction) {
    if (status) status.textContent = 'Both fields are required.';
    return;
  }
  if (status) { status.textContent = 'Submitting…'; status.style.color = 'var(--asp)'; }
  var aspect = (typeof window.currentAspect !== 'undefined') ? window.currentAspect : '';
  fetch('/learn/correct', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ query: query, correction: correction, aspect_id: aspect })
  })
  .then(function (r) { return r.json(); })
  .then(function (d) {
    if (d.ok) {
      if (status) { status.textContent = 'Corrected! Old → new fact updated.'; status.style.color = '#6dffb3'; }
      setTimeout(function () { form.remove(); }, 2000);
      showToast('Fact corrected');
    } else {
      if (status) { status.textContent = 'Error: ' + (d.error || 'unknown'); status.style.color = '#dd6666'; }
    }
  })
  .catch(function (e) {
    if (status) { status.textContent = 'Network error: ' + e.message; status.style.color = '#dd6666'; }
  });
}

// ── Init (installs callback bridge) ───────────────────────────────────────────
export function initChatRender() {
  // Install the FSM callback on window so compat.js bridge picks it up
