...
const TUTORIAL_STEPS = [
  ...
  { id: 'character_lab', title: 'Character Lab', text: 'Customize each aspect\'s personality, voice, and appearance in the Character Lab. Open it any time from the sidebar.', highlight: '[data-action="openCharacterLab"]' },
...
export function initCharacterCreator() {
  ...
  // Streamlined first-run: the 4-card setup tour (shown right after the wizard) already orients a new
  // user, so we no longer AUTO-run this separate 6-step tutorial — it was a redundant third intro that
  // used to stack on the tour/interview. It stays available on demand from the Character Lab; only the
  // automatic post-wizard launch is removed. We still load character data so the Lab is ready to open.
  loadCharacterData().catch(function () {});
}
