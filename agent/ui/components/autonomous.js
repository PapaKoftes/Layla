/**
 * components/autonomous.js — Autonomous execution monitor with real-time polling.
 *
 * Converted from js/layla-autonomous.js (top-level -> ES module).
 * Depends on: services/utils.js (showToast)
 */
...
// ── Start monitoring ────────────────────────────────────────────────────────
export function laylaAutoMonitorStart(taskId, goal) {
  _autoTaskId = taskId;
  _autoGoal = goal || '';
...
function _autoHandleProgress(data) {
  ...
  newEvents.forEach(ev => {
    if (ev.kind === 'autonomous_tool') {
      ...
      try {
        if (typeof window.operatorTraceLine === 'function') {
          window.operatorTraceLine('auto', line);
        }
      } catch (_) {}
    }
  });
...
// ── Controls ────────────────────────────────────────────────────────────────
export async function laylaAutoStop() {
...
export function laylaAutoMonitorClose() {
  clearInterval(_autoPollTimer);
  const panel = document.getElementById('auto-monitor-panel');
...
export function initAutoMonitorHook() {
  ...
  window.laylaRunAutonomousResearch = async function () {
    ...
    const runPromise = orig.apply(this, arguments);
