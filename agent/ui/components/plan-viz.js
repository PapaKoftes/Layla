/**
 * components/plan-viz.js — Plan Gantt visualization (canvas-based).
 *
 * Converted from js/layla-plan-viz.js (top-level -> ES module).
...
// ── Public entry point ──────────────────────────────────────────────────────
export async function laylaShowPlanViz(planId) {
  _vizPlanId = planId;
...
function _renderViz(data) {
  ...
  steps.forEach((s, i) => {
    ...
    const deps = s.depends_on || [];
    const startPx = deps.length
      ? Math.max(...deps.map(d => (endX[d] || 0))) * scale
      : 0;
...
async function _renderSimilarPlans(goal) {
  ...
  try {
    ...
    const items = (data.similar || []).filter(p => p.plan_id !== _vizPlanId);
