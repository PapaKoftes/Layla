...
function _onDocKeydown(e) {
  if (!_open) return;
  if (e.key === 'Escape') { e.preventDefault(); e.stopPropagation(); closeSystemDiagnostics(); }
}
...
function _build() {
  ...
  if (_escChip) {
    ...
    _escChip.addEventListener('click', () => closeSystemDiagnostics());
    _escChip.addEventListener('keydown', (e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); closeSystemDiagnostics(); } });
  }
