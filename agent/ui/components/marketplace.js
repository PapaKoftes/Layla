...
function _onDocKeydown(e) {
  if (!_open) return;
  if (e.key === "Escape") { e.preventDefault(); e.stopPropagation(); closeMarketplace(); }
}
...
function _build() {
  ...
  _root.innerHTML =
    '<div class="cmdp-panel sysdiag-panel mkt-panel" role="document">' +
      '<div class="cmdp-search-row"><span class="cmdp-search-icon" aria-hidden="true">◈</span>' +
        '<span class="sysdiag-title" data-i18n="nav.marketplace">Kit marketplace</span>' +
  ...
  if (_escChip) {
    ...
    _escChip.addEventListener("click", () => closeMarketplace());
    _escChip.addEventListener("keydown", (e) => { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); closeMarketplace(); } });
  }
