...
function _onDocKeydown(e) {
  if (!_open) return;
  if (e.key === "Escape") { e.preventDefault(); e.stopPropagation(); closeKb(); }
}
...
function _build() {
  ...
  _root.innerHTML =
    '<div class="cmdp-panel sysdiag-panel kb-panel" role="document">' +
      '<div class="cmdp-search-row"><span class="cmdp-search-icon" aria-hidden="true">❋</span>' +
        '<span class="sysdiag-title" data-i18n="nav.knowledge_base">Knowledge base</span>' +
  ...
  if (_escChip) {
    ...
    _escChip.addEventListener("click", () => closeKb());
    _escChip.addEventListener("keydown", (e) => { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); closeKb(); } });
  }
