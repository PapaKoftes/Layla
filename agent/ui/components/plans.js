...
function _onDocKeydown(e) {
  if (!_open) return;
  if (e.key === "Escape") { e.preventDefault(); e.stopPropagation(); closePlans(); }
}
...
function _build() {
  ...
  _root.innerHTML =
    '<div class="cmdp-panel sysdiag-panel plans-panel" role="document">' +
      '<div class="cmdp-search-row"><span class="cmdp-search-icon" aria-hidden="true">◈</span>' +
        '<span class="sysdiag-title" data-i18n="nav.plans">Plans &amp; projects</span>' +
  ...
  if (_escChip) {
    ...
    _escChip.addEventListener("click", () => closePlans());
    _escChip.addEventListener("keydown", (e) => { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); closePlans(); } });
  }
