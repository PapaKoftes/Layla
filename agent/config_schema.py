...
EDITABLE_SCHEMA: list[dict[str, Any]] = [
    ...
    {"key": "knowledge_rag_budget_s", "type": "number", "category": "memory", "default": 8.0, "min": 0.0, "max": 120.0, "hint": "Max seconds the first turn waits for a COLD embedder download before falling back to static reference docs (the download continues in the background). 0 = wait indefinitely."},
    ...
    {"key": "encryption_at_rest_enabled", "type": "boolean", "category": "safety", "default": True, "hint": "Encrypt HIGH-RISK memories at rest (credentials, secret keys, passwords, government IDs) with a local key. On by default — such data is rarely searched and most damaging if your disk is read. Encrypted rows are NOT keyword-searchable (recalled by relevance, not FTS). If the local key is ever lost, encrypted memories become unreadable. Degrades to plaintext with a loud warning if no key is available (never loses data)."},
    {"key": "encrypt_pii_at_rest", "type": "boolean", "category": "safety", "default": False, "hint": "Also encrypt broader PII (health, financial, contact details), not just high-risk secrets. Off by default because this content is often what you WANT to keyword-recall — encrypting it makes it relevance-only. Requires encryption_at_rest_enabled."},
